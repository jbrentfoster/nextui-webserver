...
class IndexHandler(tornado.web.RequestHandler):
    ...
    async def get(self):
        await self.render("templates/index.html", port=args.port, initial_node_name=initial_node_name)
...
class AjaxHandler(tornado.web.RequestHandler):
    ...
    async def post(self):
        ...
        if action == 'send-request':
            ...
            response = methods.add_node(request['node-name'])
            self.write(json.dumps(response))
        else:
...
class WebSocket(tornado.websocket.WebSocketHandler):
    ...
    def on_message(self, message):
        ...
        json_rpc_response = json.dumps({"method": json_rpc["method"], "response": result, "error": error},
                                       separators=(",", ":"))
